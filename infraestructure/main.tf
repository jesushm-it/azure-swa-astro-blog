...
resource "azurerm_static_web_app_custom_domain" "txt" {
...
resource "azurerm_dns_txt_record" "txt" {
  ...
  ttl                 = 300
  record {
    value = azurerm_static_web_app_custom_domain.txt.validation_token == "" ? "validated" : azurerm_static_web_app_custom_domain.txt.validation_token
  }
}
